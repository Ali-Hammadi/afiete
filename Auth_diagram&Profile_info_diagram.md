**مقدمة**
- الهدف: توثيق وتنفيذ سلسلة عمليات المصادقة (`signup` + OTP)، تسجيل الدخول، تسجيل الخروج، حذف الحساب، استعادة/إعادة تعيين كلمة المرور، تغيير كلمة المرور، وتعديل معلومات الملف الشخصي، بالإضافة إلى حالات الأخطاء والرسائل.
- ملف العمل هذا مُعد كمرجع تنفيذي ومخطط لبدء التنفيذ على مشروع Flutter الموجود.

**المتطلبات العامة**
- استخدام الـ endpoints المقدمة أدناه كما هي أو ما يقابلها في ملف endpoints في المشروع.
- كل طلبات الـ API تفترض JSON bodies وتستجيب بحالات HTTP معيارية (200، 201، 400، 401، 403، 404، 422، 500).
- آلية OTP: يتم إرسال كود إلى البريد الإلكتروني؛ خواص الكود يجب أن تُوثق (طول، صلاحية، محاولات) عندما تتوفر.

**قائمة المراحل (تنفيذ متدرج)**
1. إنشاء حساب + تحقق عبر OTP
2. تسجيل الدخول
3. تسجيل الخروج + إعادة تهيئة التطبيق
4. حذف الحساب + تنظيف البيانات
5. نسيت كلمة المرور (Forgot) + تحقق OTP + إعادة تعيين
6. تغيير كلمة المرور من الملف الشخصي
7. تعديل معلومات الملف الشخصي
8. معالجة الأخطاء، رسائل المستخدم، والحالات الشاذة

**تفصيل كل مرحلة**

**1) إنشاء حساب + OTP (Signup Flow)**
- Endpoint: `POST /api/patients/register/`
  - Body:
    {
      "user": { "nickname": "string", "email": "user@example.com", "password": "string" }
    }
- بعد نجاح الطلب: الخادم يرسل OTP إلى البريد. تطبيق جانب العميل ينتقل إلى شاشة "أدخل رمز التحقق".
- تحقق OTP:
  - Endpoint: `POST /api/users/otp/verify`
  - Body: { "email": "user@example.com", "code": "string" }
  - Success: علم تحقق مفعل؛ يسمح لإكمال الملف الشخصي.
- إعادة إرسال الكود:
  - Endpoint: `POST /api/users/otp/resend`
  - Body: { "email": "user@example.com" }
  - Client: يجب فرض انتظار صغير بين محاولات resend (مثلاً 30-60 ثانية) وعداد محاولات محدود.
- إكمال الملف الشخصي (بعد التحقق):
  - Endpoint: `POST /api/patients/profile/`
  - Body:
    {
      "user": { "birth_date": "2026-05-12", "gender": "male", "phone": "string" },
      "psychological_history": "string",
      "nickname": "string"
    }

حالات فشل شائعة ورسائل مقترحة:
- 400/422: "المدخلات غير صحيحة. تحقق من الحقول وحاول ثانية." (عرض رسائل الحقول من الـ API إن وجدت)
- 409 (مثلاً لو البريد مستخدم): "البريد الإلكتروني مسجل سابقاً. رجاء تسجيل الدخول أو استعادة كلمة المرور."
- OTP خطأ/منتهي: "رمز التحقق خاطئ أو انتهت صلاحيته. أعد المحاولة أو اطلب إعادة الإرسال."

**2) تسجيل الدخول (Login)**
- Endpoint: `POST /api/users/login/`
  - Body: { "email": "user@example.com", "password": "string" }
- Success: استجابة تحتوي على توكن المصادقة (JWT أو token) وبيانات المستخدم الأساسية.
- Client: احفظ التوكن في مخزن آمن (secure storage)، حدّث حالة التطبيق إلى "مصادق".

حالات فشل:
- 401/403: "بيانات الاعتماد غير صحيحة أو الحساب غير مفعل."
- 423 أو رسائل خاصة بالحظر: "الحساب موقوف أو بحاجة لتأكيد. اتصل بالدعم." (اعرض رسالة ودية)

**3) تسجيل الخروج (Logout)**
- Endpoint: `POST /api/users/logout/`
- Client: عند نجاح الخروج، قم بالآتي محلياً:
  - حذف التوكن من التخزين الآمن
  - تنظيف أي بيانات مستخدم مخزنة (caches, prefs)
  - إعادة تهيئة حالة التطبيق إلى "بدون حساب" — ولكن لا تنشئ حساب جديد تلقائياً
  - إعادة التوجيه إلى شاشة الترحيب / onboarding المناسبة

ملاحظات: إن كان الخادم يتطلب إرسال الـ token لإبطال الجلسة، ضمّن Authorization header.

**4) حذف الحساب (Delete Account)**
- Endpoint: `DELETE /api/users/account/delete/`
- سلوك العميل بعد الحذف:
  - حذف جميع بيانات المستخدم محلياً (including profile, cached resources)
  - إعادة تهيئة التطبيق كأنه جديد (show onboarding)
  - تأكيد للمستخدم: "تم حذف حسابك نهائياً" أو إظهار خطأ إذا فشل.

حالات فشل:
- 401: "غير مصرح. سجّل الدخول أولاً."
- 403/422: رسائل توضيحية إن لم يُسمح بالحذف.

**5) نسيت كلمة المرور (Forgot Password)**
1. اطلب إعادة تعيين: `POST /api/users/auth/forgot-password/` { "email": "string" }
   - الخادم يرسل OTP أو رابط إعادة التعيين إلى البريد.
2. تحقق OTP: `POST /api/users/auth/verify-otp/` { "email": "user@example.com", "code": "string" }
3. إعادة التعيين: `POST /api/users/auth/reset-password/` { "email": "user@example.com", "new_password": "stringst", "confirm_password": "stringst" }

حالات شاذة:
- OTP منتهي/خطأ: "رمز التحقق خاطئ أو انتهت صلاحيته." 
- كلمات المرور غير متطابقة: "حقلَي كلمة المرور غير مطابقين."

**6) تغيير كلمة المرور من الملف الشخصي**
- Endpoint: `POST /api/users/password/change`
  - Body: { "password": "string", "new_password": "string", "confirm_password": "string" }
- متطلبات: المستخدم يجب أن يكون مصدقاً (Authorization token).

حالات فشل:
- 401: "غير مصرح. سجّل الدخول أولاً."
- 400/422: "كلمة المرور الحالية غير صحيحة." أو "كلمة المرور الجديدة لا تستوفي الشروط." 

**7) تعديل معلومات الملف الشخصي**
- GET Profile: `GET /api/patients/profile/` — يستجيب ببنية:
  {
    "user": { "email": "user@example.com", "username": "string", "birth_date": "2026-05-12", "gender": "male", "phone": "string", "age": "string" },
    "psychological_history": "string",
    "nickname": "string"
  }
- تعديل Profile: `PUT/PATCH /api/patients/profile/` (نفس بنية الـ POST لكن بدون حقل البريد/username)

تحقق الحقول محلياً:
- تاريخ الميلاد: صيغة ISO `YYYY-MM-DD`
- الهاتف: تحقق نمط محلي/دولي
- الجنس: مقبول (male/female/other) إن كانت مدعومة

**8) معالجة الأخطاء وخرائط الرسائل للمستخدم**
- فكرة عامة: لا تعرض رسائل خطأ خام من الخادم (مثل Exception: ...). بدّلها إلى نصوص ودية ومهنية.
- اقتراحات لمطابقة حالات HTTP:
  - 200/201: نجاح → عرض رسالة إيجابية أو الانتقال للصفحة التالية
  - 400/422: مدخلات خاطئة → عرض رسائل الحقول أو: "تحقق من الحقول وحاول ثانية"
  - 401: مصادقة مفقودة/منتهية → "الرجاء تسجيل الدخول" (مع خيار إعادة التوجيه لشاشة Login)
  - 403: وصول مرفوض → "ليس لديك إذن لإجراء هذا الإجراء"
  - 404: مورد غير موجود → "المورد المطلوب غير موجود" أو إعادة محاولة
  - 500+: خطأ خادم → "حدث خطأ في الخادم. حاول لاحقاً"

ملاحظات خاصة بالـ OTP:
- طول الكود: افتراضياً 4-6 أرقام. إن لم يُحدّد الخادم، اطلب توثيق.
- صلاحية الكود: اعرض رسالة "انتهت صلاحية الكود" مع زر لإعادة الإرسال.
- عدد محاولات: بعد عدد محدد من المحاولات الفاشلة، اقتراح الانتظار أو التواصل مع الدعم.

**مخططات تدفّق مبسطة (Text-based)**
- Signup + OTP:
  1. User -> POST /api/patients/register/ (email, nickname, password)
  2. Server -> يرسل OTP إلى البريد
  3. Client -> شاشة إدخال الكود
  4. Client -> POST /api/users/otp/verify (email, code)
    - ناجح -> POST /api/patients/profile/ لإكمال البيانات
    - فشل -> عرض خطأ أو خيار إعادة الإرسال

- Forgot Password:
  1. Client -> POST /api/users/auth/forgot-password/ (email)
  2. Server -> يرسل OTP
  3. Client -> POST /api/users/auth/verify-otp/ (email, code)
  4. Client -> POST /api/users/auth/reset-password/ (email, new_password, confirm_password)

**خريطة الملفات المقترحة للتنفيذ في المشروع (خطية)**
- `lib/core/network/dio_factory.dart` — تحديث/تأكيد وجود Interceptors لـ Authorization و error mapping
- `lib/services/auth_service.dart` — دوال تنفذ كل Endpoint المذكور (register, otpVerify, otpResend, login, logout, forgot, verifyOtp, resetPassword, changePassword, getProfile, updateProfile, deleteAccount)
- `lib/models/auth_models.dart` — نماذج request/response
- `lib/repositories/auth_repository.dart` — تدفقات أعلى (signupFlow, resetFlow)
- `lib/blocs/` أو `lib/providers/` — إدارة الحالة لكل مرحلة
- `test/auth_flow_test.dart` — اختبارات وحدات/اندماجية

**خطوات التنفيذ العاجل (ما سأفعله الآن)**
1. إنشاء ملف هذا المستند (`Auth_diagram&Profile_info_diagram.md`) في جذر المشروع (تم الآن).
2. بعد موافقتك: أبدأ بفتح `lib/core/network/dio_factory.dart` للتأكد من إعداد Authorization interceptor ثم إنشاء `lib/services/auth_service.dart` و`lib/models/auth_models.dart` و`lib/repositories/auth_repository.dart`.

**اختبارات مقترحة سريعة**
- Unit: محاكاة استجابة API للتحقق من التعامل مع 200، 400، 401، 422.
- Integration (smoke): تسجيل مستخدم جديد -> تحقق OTP -> إكمال الملف -> تسجيل الدخول -> جلب الملف

**ملاحظات نهائية وسؤال**
- هل تفضل أن أبدأ فوراً بإنشاء ملفات الـ service/models/repository داخل `lib/` الآن أم أن تبدأ بمرحلة واجهات المستخدم أولاً؟

---
End of document.# Afiete Auth & Profile Management Flow Documentation
